...
resource "digitalocean_droplet" "denisubuntu" {
  count              = tonumber(var.zahlen.num_von_vpc)
...
resource "digitalocean_droplet" "deniscentos" {
  count              = tonumber(var.zahlen.num_von_vpc)
  image              = "centos-7-x64"
...
resource "aws_route53_record" "a_records_ubuntu" {
  ...
  type    = "A"
  records = [digitalocean_droplet.denisubuntu.*.ipv4_address[count.index]]
  ttl     = "300"
}
...
resource "aws_route53_record" "a_records_centos" {
  ...
  type    = "A"
  records = [digitalocean_droplet.deniscentos.*.ipv4_address[count.index]]
  ttl     = "300"
}
...
data "template_file" "templatefile_results" {
  ...
  vars = {
    ...
    my_first_ip_address = digitalocean_droplet.denisubuntu.*.ipv4_address[count.index]
    my_second_ip_addres = digitalocean_droplet.deniscentos.*.ipv4_address[count.index]
...
data "template_file" "templatefile_results_hosts" {
  ...
  vars = {
    ...
    results_ip_address_ubuntu = digitalocean_droplet.denisubuntu.*.ipv4_address[count.index]
    results_ip_address_centos = digitalocean_droplet.deniscentos.*.ipv4_address[count.index]
